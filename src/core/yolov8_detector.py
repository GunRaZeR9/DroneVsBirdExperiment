...
yolov8_detector.py
==================
YOLOv8 inference module for Bird vs Drone detection.

Key differences from YOLOv10:
  - Uses YOLOv8 architecture (standard NMS-based detection).
  - iou_thres is available but left at Ultralytics default — only ``conf`` needs tuning.
  - Slightly lower default confidence (0.35) since YOLOv8 calibrates scores differently.
  - Uses Ultralytics API for one-line model loading (same API as YOLOv10).
...
Usage:
    ...
    detector = YOLOv8Detector("runs/train/drone_v8/weights/best.pt")
...
class YOLOv8Detector:
    ...
    ----------
    weights : str | Path
        Path to the YOLOv8 best.pt checkpoint produced by training.
    conf : float
        Confidence threshold (default 0.35). YOLOv8 calibrates confidence
        differently from YOLOv10 — lower threshold is appropriate.
    device : str
    ...
    def detect_batch(
        ...
        """
        Run YOLOv8 inference on a batch of BGR images.
    ...
    def detect_video(
        ...
        try:
            while True:
                ...
                if display:
                    cv2.imshow("YOLOv8 Drone/Bird Detection", annotated)
    ...
    def _load_model(self) -> None:
        """Load YOLOv8 checkpoint via Ultralytics API."""
        try:
        ...
        if not self.weights.exists():
            logger.error(
                ...
                "Train first with: python scripts/train_yolov8.py"
            )
            return
        ...
        try:
            ...
            logger.info(f"YOLOv8 model loaded from {self.weights}")
...
def detect(
    ...
    **_kwargs,
    ...
    """
    Legacy-compatible wrapper around YOLOv8Detector.detect().
